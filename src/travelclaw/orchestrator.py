...
3. 按天组织行程，每天有清晰的时间线
4. 【美食模块重点要求】每餐必须推荐具体商家名称、2-3道招牌特色菜（含口味描述）、靠近哪个景点/POI、人均消费、营业时间。严禁用"某餐厅""当地特色店"等模糊表述。每天按早/午/下午茶/晚四段安排，每段2-3家不同价位选择。
5. 其他维度也要具体：具体酒店名、具体景点名、具体交通方式和费用
6. 确保景点、餐厅、交通之间的地理衔接合理（餐厅要在当天景点附近）
7. 预算汇总要清晰，包含每日美食预算小计
8. 输出一份用户可以直接使用的行程单
"""
